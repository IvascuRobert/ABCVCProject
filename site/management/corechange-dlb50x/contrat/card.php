...
/* *************************************************************************** */
{
    ...
    if ($object->id > 0)
    {
        ...
        if ($user->societe_id == 0)
        {
			...
			// Send
			if ($object->statut > 0) {
				if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->contract->contract_advance->send)) {
		...
		if ($action == 'presend')
		{
			...
			if (empty($object->ref_client)) {
				$formmail->withtopic = $outputlangs->trans('SendContractRef', '__ORDERREF__');
			} else if (! empty($object->ref_client)) {
				$formmail->withtopic = $outputlangs->trans('SendContractRef', '__ORDERREF__ (__REFCLIENT__)');
			}
			$formmail->withfile = 2;
			$formmail->withbody = 1;
