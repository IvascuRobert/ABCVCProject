...
if (! empty($hookmanager->resArray['eventarray'])) $eventarray=$eventarray+$hookmanager->resArray['eventarray'];

//var_dump($hookmanager->resArray['eventarray']);
//var_dump($eventarray);
$maxnbofchar=18;
            ...
            foreach ($eventarray[$daykey] as $index => $event)
            {
                ...
                if ($i < $maxprint || $maxprint == 0 || ! empty($conf->global->MAIN_JS_SWITCH_AGENDA))
                {
                    ...
                    else if ($event->type_code == 'BIRTHDAY')
                    {
                 	...
                 	{
                 		$numother++; $cssclass=(! empty($event->icalname)?'family_ext'.md5($event->icalname):'family_other');
