...
require_once DOL_DOCUMENT_ROOT . SUPP_PATH.'/propal/class/propal.class.php';
...
if (empty($reshook))
{
	...
	else if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->propal->supprimer)
	{
		...
		if ($result > 0) {
			header('Location: ' . DOL_URL_ROOT . SUPP_PATH.'/propal/list.php');
			exit();
		} else {
			$langs->load("errors");
		...
		if (! $error)
		{
			...
			if (GETPOST('createmode') == 'copy' && GETPOST('copie_propal'))
			{
				if ($object->fetch(GETPOST('copie_propal')) > 0) {
					...
					$object->statut = Propal::STATUS_VALIDATED;
...
if ($action == 'create')
{
	...
	if (! empty($conf->projet->enabled) && $socid > 0)
	{
		...
		print ' &nbsp; <a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/card.php?socid=' . $soc->id . '&action=create&status=1&backtopage='.urlencode($_SERVER["PHP_SELF"].'?action=create&socid='.$soc->id).'">' . $langs->trans("AddProject") . '</a>';
		print '</td>';
		print '</tr>';
	}
	...
	$head = $object->abcvc_propal_prepare_head($object);
	...
	$linkback = '<a href="' . DOL_URL_ROOT . SUPP_PATH.'/propal/list.php' . (! empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
		...
		if (! empty($conf->projet->enabled))
		{
			...
			if ($user->rights->propal->creer)
			{
				...
				if (! empty($object->fk_project)) {
					...
					$proj = new Project($db);
					$proj->fetch($object->fk_project);
					print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/card.php?id=' . $object->fk_project . '" title="' . $langs->trans('ShowProject') . '">';
					print $proj->ref;
					print '</a>';
					print '</td>';
				} else {
	...
	if ($action != 'presend') {
		...
		if (empty($reshook))
		{
			if ($action != 'statut' && $action != 'editline')
			{
				...
				{
					print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . SUPP_PATH.'/propal/action/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddAction") . '</a></div>';
				}
				// Edit
				...
				if ($object->statut == Propal::STATUS_SIGNED)
				{
					...
					{
						print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . SUPP_PATH.'/compta/facture.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddBill") . '</a></div>';
					}
