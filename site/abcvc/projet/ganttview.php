...
/*ABCVC HEADER */ 
echo $object->getABCVCHeader($object->id, 'gantt');
...
//        [TODO correct link/type abcvc project] 
...
if ($user->rights->projet->all->creer || $user->rights->projet->creer)
{
    ...
    {
        print '<a class="butAction" href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$object->id.'&action=create'.$param.'&tab=gantt&backtopage='.urlencode($_SERVER['PHP_SELF'].'?id='.$object->id).'">'.$langs->trans('AddTask').'</a>';
    }
    else
    {
...
if (count($tasksarray)>0)
{
	...
	foreach($tasksarray as $key => $val)
	{
		...
		//if ($s) $tasks[$taskcursor]['task_resources']='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/contact.php?id='.$val->id.'&withproject=1" title="'.dol_escape_htmltag($s).'">'.$langs->trans("List").'</a>';
		/* For JSGanttImproved */
 	...
 	if (! empty($conf->use_javascript_ajax))
	{
		...
		include_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/projet/ganttchart.inc.php';
		print '</div>'."\n";
	}
	else
	{
