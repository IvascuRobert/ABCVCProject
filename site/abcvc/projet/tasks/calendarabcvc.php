...
if (GETPOST("viewlist"))
{
    ...
    //print $param;
    header("Location: ".DOL_URL_ROOT.SUPP_PATH.SUPP_PATH.'/projet/tasks/list.php?'.$param);
    exit;
}
...
if ($showTaskProjet)
{
	...
	$sql.= ' p.fk_soc' ; 
	$sql.= ' FROM '.MAIN_DB_PREFIX.'abcvc_projet_task as pt';
	$sql.= ', '.MAIN_DB_PREFIX.'abcvc_projet as p';
...
if ($showTimeUse)
{
	...
	$sql.= ' FROM '.MAIN_DB_PREFIX.'abcvc_projet_task as pt';
	$sql.= ', '.MAIN_DB_PREFIX.'abcvc_projet as p';
	$sql.= ', '.MAIN_DB_PREFIX.'abcvc_projet_task_time as ptt';
	$sql.= ', '.MAIN_DB_PREFIX.'user as u';
	$sql.= ' WHERE pt.fk_projet = p.rowid';
...
if (empty($action) || $action == 'show_month')	  // View by month
{
	...
	//	echo ' </tr>';
		...
		{
			?>
			<td align="center"><?php echo $langs->trans("Day".(($i+(isset($conf->global->MAIN_START_WEEK)?$conf->global->MAIN_START_WEEK:1)) % 7)); ?></td>
			<?php
			$i++;
		}
		?>
		...
		for($iter_week = 0; $iter_week < 6 ; $iter_week++)
		{
			...
			for($iter_day = 0; $iter_day < 7; $iter_day++)
			{
				...
				if($tmpday <= 0)
				{
					...
					?>
					<td class="<?php echo $style; ?>" width="14%" valign="top"  nowrap="nowrap">
					<?php
...
//	echo '<td colspan=3>'.$langs->trans("TimeTotGap").($DurationPlanned-$DurationSpent)."H</td>";
	...
	{
		?>
		<td align="center"><?php echo $langs->trans("Day".(($i+(isset($conf->global->MAIN_START_WEEK)?$conf->global->MAIN_START_WEEK:1)) % 7)); ?></td><?php echo "\n";
		$i++;
	}
	?>
	</tr>
...
else	// View by day
{
	...
	<table width="100%" class="nocellnopadd">
		<tr class="liste_titre">
			 <td align="center"><?php echo $langs->trans("Day".$arraytimestamp['wday']); ?></td>
			 <?php
			 echo "\n";
			 ?>
		</tr>
...
function show_day_events($db, $day, $month, $year, $monthshown, $style, &$taskarray, $maxPrint=0, $maxnbofchar=16, $newparam='', $showinfo=0, $minheight=60)
{
	...
	<div id="dayevent_<?php echo sprintf("%04d",$year); echo sprintf("%02d",$month); echo sprintf("%02d",$day); ?>" class="dayevent">
	<?php
	echo "\n";
	...
	<a href="<?php echo DOL_URL_ROOT; ?>/projet/tasks/calendar.php?"
	action=show_day&day=<?php echo str_pad($day, 2, "0", STR_PAD_LEFT); ?>&month=<?php echo str_pad($month, 2, "0", STR_PAD_LEFT); ?>&year=<?php echo $year;
	echo $newparam;
	//.'&month='.$month.'&year='.$year;
	?> >
	<?php
	if ($showinfo) echo dol_print_date($curtime,'daytext');
...
//		print ($duration ? convertSecondToTime($duration, 'allhourmin'):'');
	...
	if ($user->rights->management->readuser)
	{
		...
		?>
		<a href="<?php echo DOL_URL_ROOT;?>/projet/tasks.php?action=create<?php echo $dateSel; ?> &backtopage =<?php echo urlencode($_SERVER["PHP_SELF"].($newparam?'?'.$newparam:'')); ?>">
		<?php
		echo img_picto($langs->trans("NewAction"),'edit_add.png');
		?>
		</a>
		<?php
	}
	...
	<tr height="<?php echo $minheight; ?>"><td valign="top" colspan="4" nowrap="nowrap">
	<?php
		...
		if ($day==$jour && $month==$mois && $year==$annee)
		{
			foreach ($taskarray[$daykey] as $index => $task)
			{
				if ($i < $maxPrint || $maxPrint == 0)
				{
					...
					<div id="event_<?php echo sprintf("%04d",$annee); echo sprintf("%02d",$mois); echo sprintf("%02d",$jour); ?>_<?php echo $i; ?>" class="event">
					<table class="cal_event" style="background: #<?php echo $color; ?>; -moz-border-radius:4px; " width="100%"><tr>
					<td nowrap="nowrap">
					<?php
					// Date
					...
					else
					{
						...
						{
							if ($showinfo)
								echo $langs->trans("EventOnFullDay");?><br><?php
								echo "\n";
						}
					}

					// Show title
					echo getNomUrlTask($task,0,$maxnbofchar,'cal_event');
					...
					if ($linerelatedto) ?><br><?php echo $linerelatedto;

					
					// show project 
					$projectstatic = new Project($db);
					$result=$projectstatic->fetch($task->fk_projet);
					?><br><?php echo $projectstatic->getNomUrl(1);
					...
					if ($showinfo)
					{
						if ($task->location)
						{
							?><br>
							<?php
							echo $langs->trans("Location");?> : <?php echo $task->location;
						}
					}
				...
				{
					?>
					<a href="calendar.php?maxprint=on&month=<?php echo $monthshown; ?>&year=<?php echo $year.
					($status?"&status=".$status:"").($filter?"&filter=".$filter:"").
					($filtera?"&filtera=".$filtera:"").($filtert?"&filtert=".$filtert:"").($filterd?"&filterd=".$filterd:""); ?>
					>
					<?php 
					echo img_picto("all","1downarrow_selected.png").' ...';
					echo '+'.(sizeof($taskarray[$daykey])-$maxPrint);
					?>
					</a>
					<?php
					break;
...
function getNomUrlTask($task, $withpicto=0, $maxlength=0, $classname='')
{
	...
	$result='';
	if($task->type_code=="TIMES")
		$lien = '<a '.($classname?'class="'.$classname.'" ':'').'href="'.DOL_URL_ROOT.SUPP_PATH.SUPP_PATH.'/projet/tasks/time.php?id='.$task->id.'&withproject=1">';
	else
		$lien = '<a '.($classname?'class="'.$classname.'" ':'').'href="'.DOL_URL_ROOT.SUPP_PATH.SUPP_PATH.'/projet/tasks/task.php?id='.$task->id.'">';
	$lienfin='</a>';
	//print $this->libelle;
	if ($withpicto == 2)
	{
...
<form method="POST" id="searchFormList" action="<?php echo $_SERVER["PHP_SELF"]; ?>">
    	<div class="panel panel-primary filterable">
				...
				<form name="listactionsfilter" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST">
					...
					<input type="hidden" name="showbirthday" value="<?php echo $showbirthday; ?>">
					   	</div>	
					<table class="nobordernopadding" width="100%">
							<?php
							if ($canedit || $conf->projet->enabled)
							{
								...
								<table class="nobordernopadding">
									...
									if ($canedit)
									{
										...
										<tr>
											<td nowrap="nowrap">
											<?php
											echo $langs->trans("TasksAskedBy"); ?>
											&nbsp;
											</td>
											<td nowrap="nowrap"><?php
											echo $form->select_users($filtera,'userasked',1,'',!$canedit);?>
											</td>
										</tr>
										<?php
										...
										<tr>
											<td nowrap="nowrap">
											<?php
											echo $langs->trans("or").' '.$langs->trans("TasksDoneBy");
											?>
											&nbsp;
											</td>
											<td nowrap="nowrap"><?php
											echo $form->select_users($filterd,'userdone',1,'',!$canedit);
											?>
											</td>
										</tr>
										<?php
								...
								<td align="center" valign="middle" nowrap="nowrap">
									<?php
									echo img_picto($langs->trans("ViewCal"),'object_calendar');
									?>
									<input type="submit" class="button" style="width:120px" name="viewcal" value="<?php echo $langs->trans("ViewCal"); ?>">
									<br>
									<?php
									echo img_picto($langs->trans("ViewWeek"),'object_calendarweek'); ?> <input type="submit" class="button" style="width:120px" name="viewweek" value="<?php echo $langs->trans("ViewWeek"); ?>">
									<br>
									<?php
									echo img_picto($langs->trans("ViewDay"),'object_calendarday'); ?> <input type="submit" class="button" style="width:120px" name="viewday" value="<?php echo $langs->trans("ViewDay"); ?>">
									<br>
									<?php
									echo img_picto($langs->trans("ViewList"),'object_list'); ?> <input type="submit" class="button" style="width:120px" name="viewlist" value="<?php echo $langs->trans("ViewList"); ?>">
								</td>
								<?php
								...
								<td align="left" valign="middle" nowrap="nowrap">
									<input type="hidden" id="showFilter" name="showFilter" value="1" >
									<table>
										<tr><td><input type="checkbox" id="showTaskProjet" name="showTaskProjet" value="1" <?php echo (($showTaskProjet==1)?" checked='checked'":""); ?> > <?php echo $langs->trans("AgendaShowTaskProjet"); ?></td></tr>
										<tr><td><input type="checkbox" id="showTimeUse" name="showTimeUse" value="1" <?php echo (($showTimeUse==1)?" checked='checked'":""); ?> > <?php echo $langs->trans("AgendaShowTimeUse"); ?></td></tr>
									</table>
								</td>
								</tr>
								<?php
