...
if (! empty($project_ref) && ! empty($withproject))
{
	if ($projectstatic->fetch(0,$project_ref) > 0)
	{
		...
		{
			header("Location: ".DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$projectstatic->id.($withproject?'&withproject=1':'').(empty($mode)?'':'&mode='.$mode));
			exit;
		}
	}
}
...
if (($id > 0 || ! empty($ref)) || $projectidforalltimes > 0)
{
	...
	if (empty($projectidforalltimes))
	{
		...
		$linkback=$withproject?'<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$projectstatic->id.'">'.$langs->trans("BackToList").'</a>':'';
	...
	{	
		...
		if ($resql)
		{
			...
			if (! empty($projectidforalltimes))
			{
			    ...
			    $linktotasks='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$projectstatic->id.'">'.$langs->trans("GoToListOfTasks").'</a>';
