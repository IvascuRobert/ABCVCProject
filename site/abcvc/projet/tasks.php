...
if ($action == 'createtask' && $user->rights->projet->creer)
{
	...
	if (! $cancel)
	{
		...
		if (! $error)
		{
			...
			{
				header("Location: ".DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/list.php'.(empty($mode)?'':'?mode='.$mode));
				exit;
			}
			$id = $projectid;
		}
...
/*ABCVC HEADER */ 
echo $object->getABCVCHeader($object->id, 'tasks');
...
if ($action == 'create' && $user->rights->projet->creer && (empty($object->thirdparty->id) || $userWrite > 0))
{
	...
	//$obj = empty($conf->global->PROJECT_TASK_ADDON)?'mod_taskAbcvc_simple':$conf->global->PROJECT_TASK_ADDON;
	$obj = 'mod_taskAbcvc_simple';

	//if (! empty($conf->global->PROJECT_TASK_ADDON) && is_readable(DOL_DOCUMENT_ROOT ."/core/modules/projectAbcvc/task/".$conf->global->PROJECT_TASK_ADDON.".php"))
	if (! empty($conf->global->PROJECT_TASK_ADDON) && is_readable(DOL_DOCUMENT_ROOT ."/core/modules/projectAbcvc/task/".$obj.".php"))
	
	{
		require_once DOL_DOCUMENT_ROOT ."/core/modules/projectAbcvc/task/".$obj.'.php';
	...
	print $object->selectProjectTasks(GETPOST('task_parent'),$projectid?$projectid:$object->id, 'task_parent', 0, 0, 1, 1);
	print '</td></tr>';
...
else if ($id > 0 || ! empty($ref))
{
	...
	$linktotasks='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/time.php?projectid='.$object->id.'&withproject=1">'.$langs->trans("GoToListOfTimeConsumed").'</a>';
