...
if (! empty($conf->propal->enabled))      require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/propal/class/propal.class.php';
// var_dump(DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/facture/class/facture.class.php');
if (! empty($conf->facture->enabled))     require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/facture/class/facture.class.php';
if (! empty($conf->facture->enabled))     require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/facture/class/facture-rec.class.php';
...
if (! empty($conf->deplacement->enabled)) require_once DOL_DOCUMENT_ROOT.SUPP_ABCVC.'/compta/deplacement/class/deplacement.class.php';
...
/*ABCVC HEADER */ 
echo $object->getABCVCHeader($object->id, 'element');
...
/* TABLE ASSOCIATION ACTION/CORE DOLIBARR
 * Referers types
 */
...
$listofreferent=array(
	'propal'=>array(
		...
		'table'=>'propal',
	    'datefieldname'=>'datep',
	    'urlnew'=>DOL_URL_ROOT.SUPP_PATH.'/propal/card.php?action=create&origin=project&originid='.$id.'&socid='.$socid,
	    'lang'=>'propal',
	    'buttonnew'=>'AddProp',
	...
	'invoice'=>array(
		...
		'table'=>'facture',
		'datefieldname'=>'datef',
	    'urlnew'=>DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?action=create&projectid='.$id.'&socid='.$socid,
	    'lang'=>'bills',
	...
	// 	'table'=>'facture_rec',
	// 	'datefieldname'=>'datec',
	//     'urlnew'=>DOL_URL_ROOT.SUPP_ABCVC.'/compta/facture.php?action=create&projectid='.$id.'&socid='.$socid,
	//     'lang'=>'bills',
	//     'buttonnew'=>'CreateBill',
	//     'testnew'=>$user->rights->facture->creer,
	//     'test'=>$conf->facture->enabled && $user->rights->facture->lire),
	 'proposal_supplier'=>array(
	 	...
	 	'datefieldname'=>'date',
	     'urlnew'=>DOL_URL_ROOT.'/supplier_proposal/card.php?action=create&projectid='.$id,
	...
	'order_supplier'=>array(
	 	...
	 	'datefieldname'=>'date_commande',
	     'urlnew'=>DOL_URL_ROOT.'/fourn/commande/card.php?action=create&projectid='.$id,
	     'lang'=>'suppliers',
	...
	// 	'disableamount'=>0,
	//     'urlnew'=>DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$id,
...
<?php
//[TODO override origin/type for correct abcvc tables]
...
foreach ($listofreferent as $key => $value)
{
	...
	if ($qualified)
	{
		...
		{
			$addform.='<div class="inline-block valignmiddle">';
			if ($testnew) $addform.='<a class="button" href="'.$urlnew.'">'.($buttonnew?$langs->trans($buttonnew):$langs->trans("Create")).'</a>';
			elseif (empty($conf->global->MAIN_BUTTON_HIDE_UNAUTHORIZED)) {
				$addform.='<a class="button buttonRefused" disabled="disabled" href="#">'.($buttonnew?$langs->trans($buttonnew):$langs->trans("Create")).'</a>';
			}
            $addform.='<div>';
		}
		...
		// Ref
		print '<td'.(($tablename != 'actioncomm' && $tablename != 'projet_task') ? ' style="width: 100px"':'').'>'.$langs->trans("Ref").'</td>';
		// Date
		print '<td'.(($tablename != 'actioncomm' && $tablename != 'projet_task') ? ' style="width: 100px"':'').' align="center">';
		...
		else print '<td align="right" width="100">'.$langs->trans("Status").'</td>';
		print '</tr>';
		...
		if (is_array($elementarray) && count($elementarray)>0)
		{
			...
			for ($i = 0; $i < $num; $i++)
			{
				...
				if ($tablename != 'projet_task' && $tablename != 'facture_fourn' )
				{
					// var_dump($tablename);
