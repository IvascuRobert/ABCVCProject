...
class FormProjets
{
	 ...
	 */
	function select_project($socid=-1, $selected='', $htmlname='projectid', $maxlength=16, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0, $mode = 0, $filterkey = '', $nooutput=0, $forceaddid=0)
	{
		...
		if (! empty($conf->use_javascript_ajax) && ! empty($conf->global->PROJECT_USE_SEARCH_TO_SELECT))
		{
			...
			if ($selected && empty($selected_input_value))
			{
				...
				$project = new ProjectABCVC($this->db);
				$project->fetch($selected);
				$selected_input_value=$project->ref;
			}
			$urloption='socid='.$socid.'&htmlname='.$htmlname;
			$out.=ajax_autocompleter($selected, $htmlname, DOL_URL_ROOT.SUPP_PATH.'/projet/ajax/projects.php', $urloption, $conf->global->PROJECT_USE_SEARCH_TO_SELECT, 0, array());
			$out.='<input type="hidden" name="project_'.$htmlname.'" id="project_'.$htmlname.'" value="'.$selected_input_value.'" />';
			$out.='<p class="minwidth200" id="project_label'.$htmlname.'">'.$selected_input_value.'<p/>';
		}
		else
		{
	...
	function select_projects_list($socid=-1, $selected='', $htmlname='projectid', $maxlength=24, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0, $mode=0, $filterkey = '', $nooutput=0, $forceaddid=0)
	{
		...
		{
			$projectstatic = new ProjectABCVC($this->db);
		...
		// 
		// 
		// $sql = 'SELECT p.rowid, p.ref, p.fk_projet, p.label,prj.ref,prj.title
		// 		from llx_abcvc_projet_task as p 
		// 		left join llx_abcvc_projet as prj on (p.fk_projet = prj.rowid)
		// 		where p.fk_projet = 22 and p.fk_task_parent = 0';
	...
	function select_projects_list_collaborateurs($socid=-1, $selected='', $htmlname='projectid', $maxlength=24, $option_only=0, $show_empty=1, $discard_closed=0, $forcefocus=0, $disabled=0, $mode=0, $filterkey = '', $nooutput=0, $forceaddid=0)
	{
		global $user,$conf,$langs;
		...
		{
			$projectstatic = new ProjectABCVC($this->db);
			$projectsListId = $projectstatic->getProjectsAuthorizedForUser($user,3,1);
		}
	...
	function select_post_afected_by_project_list($socid=-1, $selected='',  $maxlength=40 )
	{
		global $user,$conf,$langs;
		...
		if ($selected != '') {
			// Search all tasks afected to project selected
			$sql = 'SELECT p.rowid, p.ref, p.fk_projet, p.label,prj.ref as project_ref ,prj.title as titlul, prj.rowid as rowid_projet
				from llx_abcvc_projet_task as p 
				left join llx_abcvc_projet as prj on (p.fk_projet = prj.rowid)
				where p.fk_task_parent = 0
				and p.fk_projet ='.$selected;
		}else{
			// Search all tasks afected by project selected in facture
			$sql = 'SELECT p.rowid, p.ref, p.fk_projet, p.label,prj.ref as project_ref ,prj.title as titlul, prj.rowid as rowid_projet
					from llx_abcvc_projet_task as p 
					left join llx_abcvc_projet as prj on (p.fk_projet = prj.rowid)
					where p.fk_task_parent = 0';
		...
		if ($resql)
		{
			...
			if ($num)
			{
				while ($i < $num)
				{
					...
					{
						$labeltoshow=dol_trunc($obj->project_ref.' -> '.$obj->ref.' '.$obj->label,$maxlength);
						$labeltoshow.=' '.dol_trunc($obj->title,$maxlength);
						$out.= '<option value="'.$obj->rowid_projet.'_'.$obj->rowid.'"';
						$out.= '>'.$labeltoshow.'</option>';
					}
					$i++;
				}
	...
	function select_element($table_element, $socid=0, $morecss='', $limitonstatus=-2)
	{
		...
		if ($table_element == 'abcvc_projet_task') return '';		// Special cas of element we never link to a project (already always done)
