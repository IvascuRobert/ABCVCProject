...
function project_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/card.php?id='.$object->id;
	$head[$h][1] = $langs->trans("Project");
	$head[$h][2] = 'project';
	$h++;

    $nbContact = count($object->liste_contact(-1,'internal')) + count($object->liste_contact(-1,'external'));
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/contact.php?id='.$object->id;
	...
	{
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/element.php?id='.$object->id;
	...
	if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
    {
		...
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/note.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/document.php?id='.$object->id;
	...
	if (empty($conf->global->PROJECT_HIDE_TASKS))
	{
		...
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks.php?id='.$object->id;
		...
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/ganttview.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/info.php?id='.$object->id;
...
function task_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/task.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	$head[$h][1] = $langs->trans("Card");
	$head[$h][2] = 'task_task';
	$h++;

	$nbContact = count($object->liste_contact(-1,'internal')) + count($object->liste_contact(-1,'external'));
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/contact.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	$head[$h][1] = $langs->trans("TaskRessourceLinks");
	if ($nbContact > 0) $head[$h][1].= ' <span class="badge">'.$nbContact.'</span>';
	$head[$h][2] = 'task_contact';
	$h++;

	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/time.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	...
	if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
    {
        ...
        if(!empty($object->note_private)) $nbNote++;
		if(!empty($object->note_public)) $nbNote++;
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/note.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
		$head[$h][1] = $langs->trans('Notes');
		if ($nbNote > 0) $head[$h][1].= ' <span class="badge">'.$nbNote.'</span>';
		$head[$h][2] = 'task_notes';
		$h++;
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/document.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
...
function project_admin_prepare_head()
{
	...
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/projet/admin/project_task_extrafields.php';
...
function projectLinesa(&$inc, $parent, &$lines, &$level, $var, $showproject, &$taskrole, $projectsListId='', $addordertick=0, $projectidfortotallink=0)
{
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_parent == $parent || $level < 0)       // if $level = -1, we dont' use sublevel recursion, we show all lines
		{
			...
			if ($showline)
			{
				...
				print "<td>";
				if ($showlineingray) print '<i>';
				//else print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/task.php?id='.$lines[$i]->id.'&withproject=1">';
				for ($k = 0 ; $k < $level ; $k++)
				{
					print "&nbsp; &nbsp; &nbsp;";
				}
				...
				print '<td align="right">';
				if ($showlineingray) print '<i>';
				else print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/time.php?id='.$lines[$i]->id.($showproject?'':'&withproject=1').'">';
	...
	if (($total_projectlinesa_planned > 0 || $total_projectlinesa_spent > 0) && $level <= 0)
	{
		...
		print '</td>';
		print '<td align="right" class="nowrap liste_total">';
		if ($projectidfortotallink > 0) print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/time.php?projectid='.$projectidfortotallink.($showproject?'':'&withproject=1').'">';
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_task_parent == $parent)
		{
			...
			if (empty($mine) || ! empty($tasksrole[$lines[$i]->id]))
			{
				...
				if ($lines[$i]->duration)
				{
					print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/projet/tasks/time.php?id='.$lines[$i]->id.'">';
