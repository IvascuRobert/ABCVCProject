...
if (! empty($conf->tax->enabled) && $user->rights->tax->charges->lire)
{
	if (!$socid)
	{
		...
		if ( $resql )
		{
			...
			print '<table class="noborder" width="100%">';
			print '<tr class="liste_titre">';
			print '<td>'.$langs->trans("ContributionsToPay").($num?' <a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/sociales/index.php?status=0"><span class="badge">'.$num.'</span></a>':'').'</td>';
...
if (! empty($conf->facture->enabled) && $user->rights->facture->lire)
{
	...
	if ($resql)
	{
		...
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre"><td colspan="2">'.$langs->trans("BillsCustomersUnpaid",$num).' <a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture/list.php?search_status=1"><span class="badge">'.$num.'</span></a></td>';
