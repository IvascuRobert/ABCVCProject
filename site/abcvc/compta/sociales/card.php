...
if ($id > 0)
{
	...
	if ($result > 0)
	{
		...
		$linkback = '<a href="' . DOL_URL_ROOT . SUPP_PATH.'/compta/sociales/index.php">' . $langs->trans("BackToList") . '</a>';
		...
		if ($resql)
		{
            ...
            if ($num > 0)
            {
    		    while ($i < $num)
    		    {
    		        ...
    		        $var=!$var;
    		        print "<tr ".$bc[$var]."><td>";
    		        print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/payment_sc/card.php?id='.$objp->rowid.'">'.img_object($langs->trans("Payment"),"payment").' '.$objp->rowid.'</a></td>';
