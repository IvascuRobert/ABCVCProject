...
class Paiement extends CommonObject
{
    ...
    function addPaymentToBank($user,$mode,$label,$accountid,$emetteur_nom,$emetteur_banque,$notrigger=0)
    {
        ...
        if (! empty($conf->banque->enabled))
        {
            ...
            if ($bank_line_id > 0)
            {
                ...
                {
                    $url='';
                    if ($mode == 'payment') $url=DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/card.php?id=';
				...
				if (! $error && $label == '(WithdrawalPayment)')
				{
					$result=$acc->add_url_line(
						$bank_line_id,
						$this->id_prelevement,
						DOL_URL_ROOT.SUPP_PATH.'/compta/prelevement/card.php?id=',
						$this->num_paiement,
						'withdraw'
					);
				}
	...
	if (count($arraybill) >0)
	{
        ...
        $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/card.php?id='.$this->id.'" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
		$linkend='</a>';
