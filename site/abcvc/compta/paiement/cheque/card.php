...
if ($action == 'confirm_reject_check' && $confirm == 'yes' && $user->rights->banque->cheque)
{
	...
	if ($paiement_id > 0)
	{
		...
		//header("Location: ".DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/card.php?id='.$paiement_id);
		//exit;
		$action='';
	}
	else
...
else
{
	...
	$hselected = $h;
	$h++;
	//  $head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/cheque/info.php?id='.$object->id;
...
else
{
	...
	$linkback='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/cheque/list.php">'.$langs->trans("BackToList").'</a>';
