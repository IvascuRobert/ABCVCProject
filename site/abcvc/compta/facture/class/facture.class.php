...
/**
 *	\file       htdocs/abcvc/compta/facture/class/facture.class.php
 *	\ingroup    facture
...
class Facture extends CommonInvoice
{
	...
	/**
	 *	Create A VALID invoice  in database.
	...
	function create($user,$notrigger=0,$forceduedate=0)
	{
		...
		if ($this->fac_rec > 0)
		{
			require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/facture/class/facture-rec.class.php';
		...
		if ($resql)
		{
			...
			else	// If this->lines is an array of invoice line arrays
			{
				...
				// RECUPERARE PROJECT TREE
                    ...
                    $projectTree = $objproj->getProjectTree($this->fk_project, $user);
                    // var_dump($projectTree);
                    $projectBills = $objproj->getProjectBillingHistory($this->fk_project, $user);
                    ...
                    foreach ($projectTree['tree'] as $key => $lot) :
                        ...
                        foreach ($lot->categories as $key => $categorie) : 
                            ...
                            foreach ($categorie->postes as $key => $poste) : 
                            	$cost_progress = $this->get_cost_post_to_bill($projectBills[$poste->rowid],$poste->progress,$poste->poste_pv);

                            	//obsolete $cost_progress = $this->get_cost_post_to_bill($this->fk_project,$poste->ref,$poste->progress,$poste->poste_pv);
                            	// var_dump($cost_progress['price']);
                            	// var_dump($cost_progress['progress']);
                                $treelines[] = (object)array(

                                    'desc'=> $poste->ref." ".$poste->label,
                                    'subprice'=> $cost_progress['price'],
                                    'import_key' => $cost_progress['progress'],
                                    //$poste->poste_pv * $poste->progress)/100 ,
                                    'qty'=> 1,
                                    'tva_tx'=> $poste->tx_tva,
                                    'localtax1_tx' => 0,
                                    'localtax2_tx' => 0,
	...
	function getNomUrl($withpicto=0,$option='',$max=0,$short=0,$moretitle='',$notooltip=0,$addlinktonotes=0)
	{
		...
		if ($option == 'withdraw') $url = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/prelevement.php?facid='.$this->id;
		else $url = DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?facid='.$this->id;
		...
		if ($addlinktonotes)
		{
		    ...
		    if ($txttoshow)
		    {
    		    ...
    		    $result.='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture/note.php?id='.$this->id.'" class="classfortooltip" title="'.dol_escape_htmltag($notetoshow).'">'.img_picto('','object_generic').'</a>';
	...
	function fetch_lines()
	{
		...
		$sql.= ' p.ref as product_ref, p.fk_product_type as fk_product_type, p.label as product_label, p.description as product_desc, l.import_key';
	...
	function addline($desc, $pu_ht, $qty, $txtva, $txlocaltax1=0, $txlocaltax2=0, $fk_product=0, $remise_percent=0, $date_start='', $date_end='', $ventil=0, $info_bits=0, $fk_remise_except='', $price_base_type='HT', $pu_ttc=0, $type=self::TYPE_STANDARD, $rang=-1, $special_code=0, $origin='', $origin_id=0, $fk_parent_line=0, $fk_fournprice=null, $pa_ht=0, $label='', $array_options=0, $situation_percent=100, $fk_prev_id='', $fk_unit = null, $pu_ht_devise = 0, $import_key = null)
	{
		...
		dol_syslog(get_class($this)."::addline facid=$this->id,desc=$desc,pu_ht=$pu_ht,qty=$qty,txtva=$txtva, txlocaltax1=$txlocaltax1, txlocaltax2=$txlocaltax2, fk_product=$fk_product,remise_percent=$remise_percent,date_start=$date_start,date_end=$date_end,ventil=$ventil,info_bits=$info_bits,fk_remise_except=$fk_remise_except,price_base_type=$price_base_type,pu_ttc=$pu_ttc,type=$type, fk_unit=$fk_unit,import_key=$import_key", LOG_DEBUG);
		include_once DOL_DOCUMENT_ROOT.'/core/lib/price.lib.php';
		// if($desc == '1.1.1 Poste TEST' ){
		// 	var_dump($import_key);
		// 	exit;
		// }
	...
	function load_board($user)
	{
		...
		if ($resql)
		{
			...
			$response->url=DOL_URL_ROOT.SUPP_PATH.'/compta/facture/list.php?search_status=1&mainmenu=accountancy&leftmenu=customers_bills';
	...
	*/
	function abcvc_facture_prepare_head($object)
	{
		...
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?facid='.$object->id;
		...
		if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB))
		{
		    ...
		    $head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/contact.php?facid='.$object->id;
		...
		{
			$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/apercu.php?facid='.$object->id;
		...
		if (! empty($conf->prelevement->enabled))
		{
			...
			$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/prelevement.php?facid='.$object->id;
	    ...
	    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
	    {
	    	...
	    	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/note.php?facid='.$object->id;
		...
		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/document.php?facid='.$object->id;
		$head[$h][1] = $langs->trans('Documents');
		if (($nbFiles+$nbLinks) > 0) $head[$h][1].= ' <span class="badge">'.($nbFiles+$nbLinks).'</span>';
		$head[$h][2] = 'documents';
		$h++;

		$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/facture/info.php?facid='.$object->id;
	...
	function get_cost_post_to_bill($projectBillsPoste,$progress_poste, $poste_pv)	{

		//var_dump($projectBillsPoste);
		/* 	array (size=2)
		      'FA1711-0001' => 
		        array (size=8)
		          'facnumber' => string 'FA1711-0001' (length=11)
		          'datec' => string '2017-11-03 13:09:10' (length=19)
		          'description' => string '1.1.1 Installation de chantier suivant CCTP' (length=43)
		          'poste_pv' => string '1800.00000000' (length=13)
		          'subprice' => string '900.00000000' (length=12)
		          'total_ht' => string '900.00000000' (length=12)
		          'total_ttc' => string '1080.00000000' (length=13)
		          'import_key' => string '50' (length=2)
		      'FA1711-0002' => 
		        array (size=8)
		          'facnumber' => string 'FA1711-0002' (length=11)
		          'datec' => string '2017-11-04 11:14:15' (length=19)
		          'description' => string '1.1.1 Installation de chantier suivant CCTP' (length=43)
		          'poste_pv' => string '1800.00000000' (length=13)
		          'subprice' => string '450.00000000' (length=12)
		          'total_ht' => string '450.00000000' (length=12)
		          'total_ttc' => string '540.00000000' (length=12)
		          'import_key' => string '25' (length=2)
		*/
		if(is_array($projectBillsPoste)){
			$sum_factured = 0;
			$progress_factured = 0;
			foreach ($projectBillsPoste as $ref_bill => $projectBillPoste) {
				$sum_factured += (float)$projectBillPoste['total_ht'];
				$progress_factured += (float)$projectBillPoste['import_key'];
			}
	        $progress_to_bill = $progress_poste - $progress_factured;

		} else {
			$progress_to_bill = $progress_poste;
		}


		$to_bill = array(
			'price' => ($progress_to_bill * $poste_pv)/100,
			'progress' => $progress_to_bill
		);
		//var_dump($to_bill);
        //exit();
	...
	function obsolete_get_cost_post_to_bill($idproject,$refpost,$progress,$poste_pv)
	{
		//GET FACTUREs FACUT PENTRU IDPROJECT pe ordine de date
		
		//SUM( cost facturat pe $refpost )
		
		//CAT LA % a fost facturat 
		
		//Calculez DELTA ( $actual progres - progres calculat mai sus ) 
		
		//returneaza SUM final
		$sql = '
		SELECT sum(f.subprice ) as sum_facturat

		FROM llx_facturedet as f 
		LEFT JOIN llx_facture as fa on (fa.rowid = f.fk_facture)

		WHERE fa.fk_projet=' . $idproject  . ' AND f.description LIKE "' . $refpost  . ' %"
        ...
        { 
            ...
            $sum_facturat = $obj->sum_facturat;       

            $this->db->free($resql);	
			try {
				if(is_null($sum_facturat))
	            {

	            	// var_dump($sum_facturat);
		            // var_dump($progress);
		            // var_dump($poste_pv);
		            // exit;

	            	return array(
	            		'price' => ($progress * $poste_pv)/100,
	            		'progress' => $progress
	            		);
	            }
	            else
	            {
			       	...
			       	$procentajul_facturat = @(($sum_facturat * 100) / $poste_pv);

		            // var_dump("Suma facturat ".$sum_facturat);
		            // var_dump("% facturat ".$procentajul_facturat);
		            // var_dump("Progres actual ".$progress);
		            // procentajul_facturat == progress
		            // or regression ????
		            if($procentajul_facturat >= $progress){
		            	...
		            	return array(
			       		...
			       		$delta = $progress - $procentajul_facturat;
			       		$suma_finala = ($delta * $poste_pv)/100;
			            // var_dump(" procentaujul dintre progrese ".$procentajul_facturat);
			            // var_dump("Delta ".$delta);
			            // var_dump("Suma finala ".$suma_finala);
			            // var_dump("Poste pv ".$poste_pv);
			       		return array(
		            		'price' => $suma_finala,
		            		'progress' => $delta
		            		);
	 ...
	 *	@return	void
	 */
	function printObjectLines($projectBills, $action, $seller, $buyer, $selected=0, $dateSelector=0)
	{
		// var_dump('cimiceanga');
		...
		// Progress by Post
			print '<td class="lineprogress" align="right">'.$langs->trans('% Fact.').'</td>';

			print '<td class="lineprogress" align="right">'.$langs->trans('% Total').'</td>';
		...
		// Multicurrency
			...
			print '<td class="linecoledit" width="10" ></td>';  // No width to allow autodim
		...
		//Line extrafield
			...
			foreach ($this->lines as $line)
			{
	            ...
	            if (empty($reshook))
				{
					$this->printObjectLine($projectBills, $action,$line,$var,$num,$i,$dateSelector,$seller,$buyer,$selected,$extrafieldsline);
				}
	 ...
	 *	@return	void
	 */
	function printObjectLine($projectBills, $action,$line,$var,$num,$i,$dateSelector,$seller,$buyer,$selected=0,$extrafieldsline=0)
	{
		...
		{
			//  public 'special_code' => string '24' (length=2)
			...
			{
				//var_dump( $reldir.'/objectline_view_abcvc.tpl.php');
				//exit();
				$tpl = dol_buildpath($reldir.'/objectline_view_abcvc.tpl.php');
...
class FactureLigne extends CommonInvoiceLine
{
	...
	function fetch($rowid)
	{
		...
		$sql.= ' fd.multicurrency_total_ttc, fd.import_key,';
...
// exit();
	...
	function insert($notrigger=0, $noerrorifdiscountalreadylinked=0)
	{
		...
		if (empty($this->import_key)) $this->import_key = 'null' ;
		...
		$sql.= ' fk_multicurrency, multicurrency_code, multicurrency_subprice, multicurrency_total_ht, multicurrency_total_tva, multicurrency_total_ttc,import_key';
		$sql.= ')';
