...
else
{
	...
	if ($object->id > 0)
	{
		...
		$linkback = '<a href="' . DOL_URL_ROOT . SUPP_PATH.'/compta/facture/fiche-rec.php' . (! empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
		...
		if ($user->rights->facture->creer)
		{
				...
				else
				{
					...
					{
						print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?action=create&amp;socid='.$object->thirdparty->id.'&amp;fac_rec='.$object->id.'">'.$langs->trans("CreateBill").'</a></div>';
					}
					else
					{
			...
			if ($num > 0)
			{
				...
				while ($i < min($num,$limit))
				{
					...
					if ($user->rights->facture->creer)
					{
				        ...
				        {
                            print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?action=create&amp;socid='.$objp->socid.'&amp;fac_rec='.$objp->facid.'">';
