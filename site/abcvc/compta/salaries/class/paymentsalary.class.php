...
class PaymentSalary extends CommonObject
{
		...
		if ($result)
		{
			...
			if ($this->id > 0)
			{
				if (! empty($conf->banque->enabled) && ! empty($this->amount))
				{
					...
					{
						// Add link 'payment_salary' in bank_url between payment and bank transaction
						$url=DOL_URL_ROOT.SUPP_PATH.'/compta/salaries/card.php?id=';
	...
	function getNomUrl($withpicto=0,$option='')
	{
        ...
        $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/salaries/card.php?id='.$this->id.'" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
		$linkend='</a>';
