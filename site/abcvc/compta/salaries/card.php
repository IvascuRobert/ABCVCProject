...
if ($action == 'delete')
{
	...
	if ($object->rappro == 0)
	{
		...
		if ($ret > 0)
		{
			...
			{
				$db->commit();
				header("Location: ".DOL_URL_ROOT.SUPP_PATH.'/compta/salaries/index.php');
				exit;
			}
			else
			{
...
if ($id)
{
    ...
    $linkback = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/salaries/index.php'.(! empty($socid)?'?socid='.$socid:'').'">'.$langs->trans("BackToList").'</a>';
