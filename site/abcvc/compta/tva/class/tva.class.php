...
class Tva extends CommonObject
{
        ...
        if ($result)
        {
            ...
            if ($this->id > 0)
            {
				...
				if (! empty($conf->banque->enabled) && ! empty($this->amount))
                {
                    ...
                    // Update links
                    $result=$acc->add_url_line($bank_line_id, $this->id, DOL_URL_ROOT.SUPP_PATH.'/compta/tva/card.php?id=', "(VATPayment)", "payment_vat");
                    if ($result < 0)
                    {
                    	$this->error=$acc->error;
                    	$ok=0;
	...
	function getNomUrl($withpicto=0,$option='')
	{
        ...
        $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/tva/card.php?id='.$this->id.'" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
		$linkend='</a>';
