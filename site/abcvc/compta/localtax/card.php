...
require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/localtax/class/localtax.class.php';
...
if ($action == 'delete')
{
	...
	if ($localtax->rappro == 0)
	{
	    ...
	    if ($ret > 0)
	    {
			...
			{
				$db->commit();
				header("Location: ".DOL_URL_ROOT.SUPP_PATH.'/compta/localtax/reglement.php?localTaxType='.$localtax->ltt);
				exit;
			}
			else
			{
...
{
	$h = 0;
	$head[$h][0] = DOL_URL_ROOT.SUPP_PATH.'/compta/localtax/card.php?id='.$vatpayment->id;
