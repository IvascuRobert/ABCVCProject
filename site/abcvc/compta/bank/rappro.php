...
require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/paiement/cheque/class/remisecheque.class.php';
...
if ($action == 'rappro' && $user->rights->banque->consolidate)
{
    ...
    if (! $error)
    {
		header('Location: '.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/rappro.php?account='.$id);	// To avoid to submit twice and allow back
    	exit;
    }
}
...
if ($resql)
{
    ...
    if ($resqlr)
    {
        ...
        while (($i < $numr) && ($i < $nbmax))
        {
            ...
            $i++;
            $liste='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/releve.php?account='.$acct->id.'&amp;num='.$objr->num_releve.'">'.$objr->num_releve.'</a> &nbsp; '.$liste;
        }
...
//         print "<input type=\"hidden\" name=\"rowid\" value=\"".$objp->rowid."\">";
		...
		// Description
        print '<td valign="center"><a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid='.$objp->rowid.'&amp;account='.$acct->id.'">';
		$reg=array();
        ...
        else
        {
            ...
            if ($user->rights->banque->modifier)
            {
                ...
                print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid='.$objp->rowid.'&amp;account='.$acct->id.'&amp;orig_account='.$acct->id.'">';
                print img_edit();
                print '</a>&nbsp; ';

                $now=dol_now();
                if ($db->jdate($objp->do) <= $now) {
                    print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/rappro.php?action=del&amp;rowid='.$objp->rowid.'&amp;account='.$acct->id.'">';
                    print img_delete();
                    print '</a>';
                }
                else {
