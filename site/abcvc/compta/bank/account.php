	...
	if ($action != 'delete')
	{
		...
		if ($object->canBeConciliated() > 0) {
			...
			if ($user->rights->banque->consolidate) {
				print '<a class="butAction" href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/rappro.php?account='.$object->id.($vline ? '&amp;vline='.$vline : '').'">'.$langs->trans("Conciliate").'</a>';
			} else {
	...
	if ($result)
	{
		...
		while ($i < $num)
		{
			...
			if ($i >= ($viewline * (($totalPages-$page)-1)))
			{
				...
				{
					dol_include_once(SUPP_PATH.'/compta/paiement/cheque/class/remisecheque.class.php');
				...
				if ($objp->rappro && $object->canBeConciliated() > 0)  // If line not conciliated and account can be conciliated
				{
					...
					print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid='.$objp->rowid.'&amp;account='.$object->id.'&amp;page='.$page.'">';
					print img_edit();
					print '</a>';
					print "&nbsp; ";
