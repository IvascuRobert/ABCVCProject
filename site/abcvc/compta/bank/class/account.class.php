...
class Account extends CommonObject
{
    ...
    function load_board(User $user, $filteraccountid = 0)
    {
        ...
        if ($resql)
        {
	        ...
	        $response->url=DOL_URL_ROOT.SUPP_PATH.'/compta/bank/index.php?leftmenu=bank&amp;mainmenu=bank';
    ...
    function getNomUrl($withpicto=0, $mode='')
    {
        ...
        if (empty($mode))
        {
            $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/card.php?id='.$this->id.$linkclose;
            $linkend='</a>';
        }
        else if ($mode == 'transactions')
        {
            $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/bankentries.php?id='.$this->id.$linkclose;
            $linkend='</a>';
        }
        else if ($mode == 'receipts')
        {
            $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/releve.php?account='.$this->id.$linkclose;
            $linkend='</a>';
        }
...
class AccountLine extends CommonObject
{
    ...
    function getNomUrl($withpicto=0,$maxlen=0,$option='')
    {
        ...
        $link = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid='.$this->rowid.'" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $linkend='</a>';
