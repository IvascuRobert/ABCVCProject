...
if ($action == 'add')
{
	...
	if (! $error)
	{
		...
		if (($accountto->id != $accountfrom->id) && ($accountto->currency_code == $accountfrom->currency_code))
		{
		    ...
		    if (! $error) $result=$accountfrom->add_url_line($bank_line_id_from, $bank_line_id_to, DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid=', '(banktransfert)', 'banktransfert');
			if (! ($result > 0)) $error++;
		    if (! $error) $result=$accountto->add_url_line($bank_line_id_to, $bank_line_id_from, DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid=', '(banktransfert)', 'banktransfert');
			if (! ($result > 0)) $error++;
