...
require_once DOL_DOCUMENT_ROOT.SUPP_PATH.'/compta/salaries/class/paymentsalary.class.php';
...
if ($id > 0 || ! empty($ref))
{
    ...
    $linkback = '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/index.php">'.$langs->trans("BackToList").'</a>';
    ...
    if ($action != 'addline' && $action != 'reconcile')
    {
        ...
        if ($object->canBeConciliated() > 0) {
            ...
            if ($user->rights->banque->consolidate) {
                print '<a class="butAction" href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/bankentries.php?action=reconcile'.$param.'">'.$langs->trans("Conciliate").'</a>';
            } else {
...
//	    print '<td>';
	    ...
	    if ($resqlr)
	    {
	        ...
	        while (($i < $numr) && ($i < $nbmax))
	        {
	            ...
	            $i++;
	            $liste='<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/releve.php?account='.$id.'&amp;num='.$objr->num_releve.'">'.$objr->num_releve.'</a> &nbsp; '.$liste;
	        }
...
//	    print '</td></tr></table>';
    ...
    while ($i < min($num,$limit)) 
    {
    	...
    	{
    	    print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/bank/ligne.php?rowid='.$objp->rowid.'&amp;account='.$objp->bankid.'&amp;page='.$page.'">';
    	    print img_edit();
    	    print '</a>';
    	}
    	else
