...
if (empty($reshook))
{
	...
	if ($action == 'confirm_paiement' && $confirm == 'yes')
	{
	    ...
	    if (! $error)
	    {
	        ...
	        if ($invoiceid > 0) $loc = DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?facid='.$invoiceid;
	        else $loc = DOL_URL_ROOT.SUPP_PATH.'/compta/paiement/card.php?id='.$paiement_id;
	        header('Location: '.$loc);
	        exit;
	    }
	    else
		...
		if ($conf->use_javascript_ajax)
		{
			...
			print '$(document).ready(function () {
						...
						function callForResult(imgId)
						{
							...
							$.post("'.DOL_URL_ROOT.SUPP_PATH.'/compta/ajaxpayment.php", json, function(data)
							{
								json = $.parseJSON(data);
...
if (! GETPOST('action'))
{
    ...
    if ($resql)
    {
        ...
        while ($i < min($num,$limit))
        {
            ...
            $var=!$var;
            print '<tr '.$bc[$var].'>';
            print '<td><a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?facid='.$objp->facid.'">'.$objp->facnumber."</a></td>\n";
