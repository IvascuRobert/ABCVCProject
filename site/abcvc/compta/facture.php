...
require_once DOL_DOCUMENT_ROOT . SUPP_PATH.'/compta/paiement/class/paiement.class.php';
...
if (empty($reshook))
{
	...
	// Delete invoice
		...
		if ($result > 0) {
			header('Location: ' . DOL_URL_ROOT . SUPP_PATH.'/compta/facture/list.php');
			exit();
		} else {
			setEventMessages($object->error, $object->errors, 'errors');
			$action='';
...
if (! empty($conf->projet->enabled)) { 
	...
	//ABCVC override
	$projectObj = new ProjectABCVC($db); 
	// var_dump($projectObj);
} 
...
if ($action == 'create')
{
	...
	// Ref
	print '<tr><td class="titlefieldcreate fieldrequired">' . $langs->trans('Ref') . '</td><td colspan="2">' . $langs->trans('Validate') . '</td></tr>';
	...
	if ($soc->id > 0 && ! GETPOST('fac_rec'))
	{
		...
		$outstandingBills = get_OutstandingBill($object->id);
	...
	print '<input type="submit" class="button" name="bouton" value="' . $langs->trans('Validate') . '">';
	print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
else if ($id > 0 || ! empty($ref))
{
	    ...
	    if ($user->rights->facture->creer)
	    {
	        if ($action != 'classify')
	            ...
	            if ($action == 'classify') {
	                ...
	                // ABCVC override
					$morehtmlref.=$projectObj->form_project($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->socid, $object->fk_project, 'none', 0, 0, 0, 1); 
	            }
	    } else {
	        if (! empty($object->fk_project)) {
	            $proj = new ProjectABCVC($db);
	...
	$addcreditnote = '<a href="' . DOL_URL_ROOT . SUPP_PATH.'/compta/facture.php?action=create&socid=' . $soc->id . '&type=2&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?facid=' . $object->id . '">' . $langs->trans("AddCreditNote") . '</a>';
	...
	if (!empty($conf->incoterm->enabled))
	{
        ...
        if ($user->rights->facture->creer) print '<a href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture.php?facid='.$object->id.'&action=editincoterm">'.img_edit().'</a>';
        else print '&nbsp;';
        print '</td></tr></table>';
        print '</td>';
        print '<td>';
...
//var_dump($object->fk_project);
	...
	if (! empty($object->lines))
		$ret = $object->printObjectLines($projectBills, $action, $mysoc, $soc, $lineid, 1);
...
//***********************************************************************************************
			...
			if ($object->statut > Facture::STATUS_DRAFT && $object->paye == 0 && $num == 0)
			{
			    if ($resteapayer > 0)
			    {
			        if ($user->rights->prelevement->bons->creer)
			        {
			            ...
			            print '<a class="butAction" href="'.DOL_URL_ROOT.SUPP_PATH.'/compta/facture/prelevement.php?facid='.$object->id.'" title="'.dol_escape_htmltag($langs->trans("MakeWithdrawRequest")).'">'.$langs->trans("MakeWithdrawRequest").'</a>';
			        }
			        else
			        {
     ...
     */
    function get_OutstandingBill($idfacture)
    {
    	global $db, $user, $conf;
