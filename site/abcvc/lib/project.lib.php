...
function project_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/card.php?id='.$object->id;
	$head[$h][1] = $langs->trans("Project");
	$head[$h][2] = 'project';
	$h++;

    $nbContact = count($object->liste_contact(-1,'internal')) + count($object->liste_contact(-1,'external'));
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/contact.php?id='.$object->id;
	...
	{
		$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/element.php?id='.$object->id;
	...
	if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
    {
		...
		$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/note.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/document.php?id='.$object->id;
	...
	if (empty($conf->global->PROJECT_HIDE_TASKS))
	{
		...
		$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks.php?id='.$object->id;
		...
		$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/ganttview.php?id='.$object->id;
	...
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/info.php?id='.$object->id;
...
function task_prepare_head($object)
{
	...
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks/task.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	$head[$h][1] = $langs->trans("Card");
	$head[$h][2] = 'task_task';
	$h++;

	$nbContact = count($object->liste_contact(-1,'internal')) + count($object->liste_contact(-1,'external'));
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks/contact.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	$head[$h][1] = $langs->trans("TaskRessourceLinks");
	if ($nbContact > 0) $head[$h][1].= ' <span class="badge">'.$nbContact.'</span>';
	$head[$h][2] = 'task_contact';
	$h++;

	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks/time.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
	...
	if (empty($conf->global->MAIN_DISABLE_NOTES_TAB))
    {
        ...
        if(!empty($object->note_private)) $nbNote++;
		if(!empty($object->note_public)) $nbNote++;
		$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks/note.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
		$head[$h][1] = $langs->trans('Notes');
		if ($nbNote > 0) $head[$h][1].= ' <span class="badge">'.$nbNote.'</span>';
		$head[$h][2] = 'task_notes';
		$h++;
	...
	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/tasks/document.php?id='.$object->id.(GETPOST('withproject')?'&withproject=1':'');
...
function project_timesheet_prepare_head($mode)
{
	...
	{
		$head[$h][0] = DOL_URL_ROOT."/abcvc/projet/activity/perweek.php".($mode?'?mode='.$mode:'');
	...
	// {
	// 	$head[$h][0] = DOL_URL_ROOT."/abcvc/projet/activity/perday.php".($mode?'?mode='.$mode:'');
...
function project_admin_prepare_head()
{
	...
	$head[$h][0] = DOL_URL_ROOT."/abcvc/projet/admin/project.php";
	...
	$head[$h][0] = DOL_URL_ROOT."/abcvc/projet/admin/project_extrafields.php";
	$head[$h][1] = $langs->trans("ExtraFieldsProject");
	$head[$h][2] = 'attributes';
	$h++;

	$head[$h][0] = DOL_URL_ROOT.'/abcvc/projet/admin/project_task_extrafields.php';
...
function projectLinesa(&$inc, $parent, &$lines, &$level, $var, $showproject, &$taskrole, $projectsListId='', $addordertick=0, $projectidfortotallink=0)
{
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_parent == $parent || $level < 0)       // if $level = -1, we dont' use sublevel recursion, we show all lines
		{
			...
			if ($showline)
			{
				...
				print '<td align="right">';
				if ($showlineingray) print '<i>';
				else print '<a href="'.DOL_URL_ROOT.'/abcvc/projet/tasks/time.php?id='.$lines[$i]->id.($showproject?'':'&withproject=1').'">';
	...
	if (($total_projectlinesa_planned > 0 || $total_projectlinesa_spent > 0) && $level <= 0)
	{
		...
		print '</td>';
		print '<td align="right" class="nowrap liste_total">';
		if ($projectidfortotallink > 0) print '<a href="'.DOL_URL_ROOT.'/abcvc/projet/tasks/time.php?projectid='.$projectidfortotallink.($showproject?'':'&withproject=1').'">';
...
// 				if ($lines[$i]->duration)
// 				{
// 					print '<a href="'.DOL_URL_ROOT.'/abcvc/projet/tasks/time.php?id='.$lines[$i]->id.'">';
...
// 				if($lines[$i]->fk_parent == 0){
// 				$tableCell = '<input align="right" class="starthour_input_per_day pisica">
// 							  <input align="right" class="endhour_input_per_day pisica">';
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_task_parent == $parent)
		{
			...
			if ( 
				...
				( empty($mine) || ! empty($tasksrole[$lines[$i]->id]) )
				//||
				...
				)
			{
			    ...
			    //var_dump( "idx $i / $numlines  --  array for projectid:".$projectstatic->id." -- ".$workloadforid[$projectstatic->id]);

			    if (empty($workloadforid[$projectstatic->id]))
			    {

					//var_dump( "loadTimeSpent( ?");

				    $projectstatic->loadTimeSpent($firstdaytoshow, 1, $fuser->id);	// Load time spent from table projet_task_time for the project into this->weekWorkLoad and this->weekWorkLoadPerTask for all days of a week
                    $workloadforid[$projectstatic->id]=1;
			    }
				print "<tr ".$bc[$var].">\n";
				...
				for ($idw = 0; $idw < 7; $idw++)
			    {
			    	...
			    	$dayNote = $projectstatic->weekNotePerTask[$tmpday][$lines[$i]->id];
	                ...
	                if ($alreadyspent)  
	                	{
	                    	...
	                    	$tableCell.='<span class="timesheetalreadyrecorded"><input type="text" align="left" class="center smallpadd" size="2" disabled id="timespent['.$inc.']['.$idw.']" name="task['.$lines[$i]->id.']['.$idw.']" value="'.$alreadyspent.'"></span>';
	                		...
	                		$tableCell.='<div class="timesheetalNOTreadyrecorded"></div>';
						}
			        ...
			        if($lines[$i]->fk_parent == 0)
			        	{
							...
							if($isnote == '0'){
								$isacomment = '<i class="fa fa-comment-o" aria-hidden="true"></i>';
							}else {
								$isacomment = '<i class="fa fa-commenting-o" aria-hidden="true"></i>';
							}

							
		                    $tableCell.=' <a href="#" class="bt_timespent_note" title="Gestion note"
		                    				data-task = "'.$lines[$i]->id.'" 
											data-day = "'.$idw.'" 
											data-isnote = "'. $isnote  .'" 
		                    				>'.$isacomment.'</a>';
		                    			
		                    $tableCell.=' <textarea class="visible" id="'.$lines[$i]->id.'_'.$idw.'" name="note['.$lines[$i]->id.']['.$idw.']">'.$dayNote.'</textarea>';
			...
			if ($lines[$i]->id > 0) 
			{
				...
				/*if( ! empty($tasksrole[$lines[$i]->id]) ) {
					...
					projectLinesPerWeek($inc, $firstdaytoshow, $fuser, $lines[$i]->id, $lineswithoutlevel0, $level, $projectsrole, $tasksrole, $mine, $restricteditformytask, $var);
			    ...
			    if ($parent == 0) {
			    	...
			    	projectLinesPerWeek($inc, $firstdaytoshow, $fuser, $lines[$i]->id, $lines, $level, $projectsrole, $tasksrole, $mine, $restricteditformytask, $var);
			    }
			}
			$level--;
		}
...
function print_projecttasks_array($db, $form, $socid, $projectsListId, $mytasks=0, $statut=-1, $listofoppstatus=array(),$hiddenfields=array())
{
	...
	{
		$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."abcvc_projet_task as t ON p.rowid = t.fk_projet";
	}
	...
	$sql2.= " FROM ".MAIN_DB_PREFIX."abcvc_projet as p";
