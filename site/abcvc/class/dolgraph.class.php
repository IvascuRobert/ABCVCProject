...
class DolGraph
{
	 ...
	 * @return	integer|null
	 */
	function draw($file,$fileurl='',$bar_width=1)
	{
		if (empty($file))
		{
		...
		call_user_func_array(array($this,$call), array($file,$fileurl,$bar_width));
	}
	 ...
	 * @return	void
	 */
	private function draw_jflot($file,$fileurl,$bar_width)
	{
		global $artichow_defaultfont;
		...
		else
		{
			...
			while ($i < $nblot)
			{
				...
				if (! isset($this->type[$i]) || $this->type[$i] == 'bars') {
					...
					//($i==$firstlot?'center':'left')
					$this->stringtoshow.='bars: { show: true, align: "'.$align.'", barWidth: '.$bar_width.' }, ';
				}
			...
			$this->stringtoshow.="\n".' ], { series: { stack: stack, lines: { fill: false, steps: steps }, bars: { barWidth: '.$bar_width.' } }'."\n";
