...
if (!empty($conf->projet->enabled)) {
	...
	require_once DOL_DOCUMENT_ROOT.'/abcvc/projet/class/html.formprojet.class.php';
}
...
if (empty($reshook))
{
	...
	elseif ($action == 'add' && $user->rights->fournisseur->facture->creer)
	{
		...
		if ($_POST['type'] == FactureFournisseur::TYPE_STANDARD || $_POST['type'] == FactureFournisseur::TYPE_DEPOSIT)
		{
			...
			if (! $error)
			{
				// ABCVC OVERRIDE
				$tmpproject = GETPOST('projectid', 'int');
				$array_project	= GETPOST('project_poste');
				$array_project = explode('_',$array_project);
				$tmpproject = $array_project[0];			
				$tmppost = $array_project[1];
	...
	//ABCVC OVERRIDE
	elseif ($action == 'modify_project')
	{

	    $object->fetch($id);
	    // ABCVC OVERRIDE
	   
	 	//VALUES WHICH IS SELECTED(idProject_idPoste)
		$array_project	= GETPOST('update_project_poste');
		$array_project = explode('_',$array_project);
		$tmpproject = $array_project[0];			
		$tmppost = $array_project[1];
		// var_dump("IDProject ".$tmpproject);
		// var_dump("IDPoste ".$object->poste_rowid);
		// var_dump($object);
		// exit;

	    $result=$object->set_Project_Poste($tmpproject,$tmppost,$object->poste_rowid);
	}
...
if ($action == 'create')
{
	...
	*/
	...
	if (! empty($conf->projet->enabled))
	{
		...
		//ABCVC OVERRIDE
		if($projectid != ''){
			print $formproject->select_project((empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS)?$societe->id:-1), $projectid);
		}else{
			print $formproject->select_projects((empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS)?$societe->id:-1), $projectid);
		}

		print '</td></tr>';
		print '<tr><td>Poste</td><td colspan="2">';
		// $formproject->select_projects((empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS)?$societe->id:-1), $projectid, 'projectid', 0, 0, 1, 1);
		// 
		// 
		//ABCVC OVERRIDE
			print '<select name="project_poste">';
			print '<option value="null_null">Sélectionner un poste</option>';
			print $formproject->select_post_afected_by_project_list((empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS)?$societe->id:-1), $projectid);
			print '</select>';
    	...
    	if (! empty($conf->projet->enabled))
    	{
    	    ...
    	    if ($user->rights->fournisseur->commande->creer)
    	    {
    	        ...
    	        if (! empty($object->fk_project)) {
    	            ...
    	            $morehtmlref.='<a href="'.DOL_URL_ROOT.'/abcvc/projet/card.php?id=' . $object->fk_project . '" title="' . $langs->trans('ShowProject') . '">';
