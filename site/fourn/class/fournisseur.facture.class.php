...
class FactureFournisseur extends CommonInvoice
{
    ...
    public function create($user)
    {
        ...
        if ($resql)
        {
            ...
            if ($result > 0)
            {
                ...
                $resql = $this->db->query($fact_fourn_sql);   
                if ($resql) {
                    $obj_result = $this->db->fetch_object($resql);
                    //$this->db->free($resql);
                    // var_dump($obj_result);
                }else{
                    $this->error=$this->db->lasterror();
                ...
                if($obj_result->fact_fourn == '')
                {
                    $imp_result = $this->id;
                }else{
                    //EXPLODE fact_fourn 
                    $exp_result = explode(',',$obj_result->fact_fourn);     
                    //insert neww fact fourn
                    array_push($exp_result,$this->id);
                    //implode fact_fourn 
                    $imp_result = implode(',',$exp_result);
                }

                
                //update db llx_abcvc_tasks
                $update_sql = 'UPDATE '.MAIN_DB_PREFIX."abcvc_projet_task SET fact_fourn='".$imp_result."' WHERE rowid=".$id_poste;
                dol_syslog(get_class($this)."::create", LOG_DEBUG);
    ...
    public function fetch($id='',$ref='')
    {
        ...
        $sql.= ' (  SELECT CONCAT(pt.ref," ",pt.label,"_",pt.rowid)
                    FROM llx_abcvc_projet_task as pt
                    WHERE FIND_IN_SET (t.rowid, pt.fact_fourn) limit 1
                 )   as label_poste, 
                 (SELECT ap.ref
                 FROM llx_abcvc_projet as ap
                 WHERE ap.rowid = t.fk_projet
                 )   as project_name '; //SUBSELECT WHICH show Poste affected by a fact fourn 
        ...
        if ($resql)
        {
            if ($this->db->num_rows($resql))
            {
                ...
                //New variable with NAme of the post
                //
                $tmplabelposte = explode("_",$obj->label_poste);

                $this->label_poste          = $tmplabelposte[0];
                $this->poste_rowid          = $tmplabelposte[1];
