...
if (! empty($conf->projet->enabled)) { $formproject = new FormProjets($db); //ABCVC override
    ...
    if (! empty($conf->projet->enabled))
    {
        ...
        if ($user->rights->propal->creer)
        {
            ...
            if ($action == 'classify') {
                ...
                //ABCVC override
				//$morehtmlref.=$formproject->form_project($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->socid, $object->fk_project, 'none', 0, 0, 0, 1);
				// ABCVC OVERRIDE
                if ($object->fk_project) {
	                $projet = new ProjectABCVC($db);
	                $projet->fetch($object->fk_project);
	                //print '<a href="'.DOL_URL_ROOT.'/projet/card.php?id='.$selected.'">'.$projet->title.'</a>';
	                $morehtmlref.=$projet->getNomUrl(0,'',1);
	            } else {
	                $morehtmlref.="&nbsp;";
	            }
            }
