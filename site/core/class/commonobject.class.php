...
abstract class CommonObject
{
     ...
     *	@return		int						<0 if KO, >0 if OK
     */
    function set_Project_Poste($projectid,$id_poste,$current_id_poste)
    {
        if (! $this->table_element)
        {
            dol_syslog(get_class($this)."::setProject was called on objet with property table_element not defined",LOG_ERR);
        ...
        $sql.= ' WHERE rowid = '.$this->id;
        // var_dump("De schimbat in baza fk_project cu IDProject= ".$projectid); 
        // var_dump("De schimbat in baza fact_fourn cu IDPoste= ".$id_poste); 
        // var_dump("Postul la care ne aflam ".$current_id_poste); 
        // var_dump("Factura cu ID-ul ".$this->id);
        // exit; 
    	dol_syslog(get_class($this)."::setProject", LOG_DEBUG);
        if ($this->db->query($sql))
        {
            ...
            //Post case
            if($current_id_poste != ''){
            	// var_dump("Avem valoare in current_id_poste");
            	// exit;
	            if( $id_poste == 'null' ){
	            	//case 1 : id_poste == null -> remove fact_fourn from array
	            	$fact_fourn_sql = 'SELECT fact_fourn from llx_abcvc_projet_task where rowid = '.$current_id_poste ;
		            $reql_ff = $this->db->query($fact_fourn_sql);   
		            if ($reql_ff) {
		                $obj_result = $this->db->fetch_object($reql_ff);
		                //$this->db->free($reql_ff);
		            }else{
		                $this->error=$this->db->lasterror();
		                $this->db->rollback();
		                return -1;
	            	}
	            	$tmpfactfourn = explode(",",$obj_result->fact_fourn);
	            		// var_dump("Vectorul fact_fourn initial: ".$tmpfactfourn);
	            	if ( ($i = array_search($this->id, $tmpfactfourn)) !== false )  // here remove id_fact_fourn from array
	            	{
	            		unset($tmpfactfourn[$i]);
	            		// var_dump("fact_fourn id care trebuie scos: ".$tmpfactfourn[$i]);
	            	}
	            	$tmpfactfourn_final = implode(",",$tmpfactfourn); // create new fact_fourn array
		            // var_dump("Vectorul fact_fourn final: ".$factfour_final);
		            // exit;
	            	//UPDATE fact_fourn table with new array
	            	$update_sql = 'UPDATE '.MAIN_DB_PREFIX."abcvc_projet_task SET fact_fourn = '".$tmpfactfourn_final."' WHERE rowid=".$current_id_poste;
		            // var_dump($update_sql);
		            // exit;
		            dol_syslog(get_class($this)."::create", LOG_DEBUG);
		            $resql=$this->db->query($update_sql);
		            if (! $resql) $error++;
	            }else{
	            	//case 2 : id_poste == Value selected -> add to fact_fourn array 
	  	            //SELECT  fact_fourn EXISTENT BY id_poste 
	            	$fact_fourn_sql = 'SELECT fact_fourn from llx_abcvc_projet_task where rowid = '.$current_id_poste ;
	            	// var_dump($fact_fourn_sql);
		            $resql_ff = $this->db->query($fact_fourn_sql);  
		            if ($resql_ff) {
		                $obj_result = $this->db->fetch_object($resql_ff);
		                //$this->db->free($resql);
		            }else{
		                $this->error=$this->db->lasterror();
		                $this->db->rollback();
	            	...
	            	$tmpfactfourn_scos = explode(",",$obj_result->fact_fourn);
	            	// var_dump("Fact fourn array of ids: ".$tmpfactfourn_scos[0]." ".$tmpfactfourn_scos[1]);
	            	// exit;
	            	if( ($i = array_search($this->id, $tmpfactfourn_scos)) !== false ) // here remove id_fact_fourn from array
	            	{
	            		// var_dump("DAR AICI?");
	            		$scoate = $tmpfactfourn_scos[$i];
	            		unset($tmpfactfourn_scos[$i]);
	            		// var_dump("Fact fourn ce trebuie afectat".$scoate);
	            		// var_dump("Fact fourn ce trebuie afectat".$id_poste);
	            		$sql_ff = 'SELECT fact_fourn from llx_abcvc_projet_task where rowid = '.$id_poste ;
			            $resql_ffs = $this->db->query($sql_ff);   
			            if ($resql_ffs) {
			                $obj_result = $this->db->fetch_object($resql_ffs);
			                //$this->db->free($resql);
			            }else{
			                $this->error=$this->db->lasterror();
			                $this->db->rollback();
		            	...
		            	if($tmpfactfourn[0] == '')
		            	{
		            		$tmpfactfourn[0] = $scoate;	
		            		// var_dump("Daca vectorul fact_fourn este gol intra aici!");
		            		// exit;
		            	}else{
		            		$tmpfactfourn[] = $scoate; 
		            		// var_dump("Daca vectorul fact_fourn are elemente intra aici!");
		            		// exit;
		            	}
		            	$tmpfactfourn_final = implode(",",$tmpfactfourn);
	            		// var_dump("Noul vector unde s-a adaugat id-ul ".$scoate." fact_fourn ".$tmpfactfourn_final);
	            		// exit;
		            	$update_sql = 'UPDATE '.MAIN_DB_PREFIX."abcvc_projet_task SET fact_fourn='".$tmpfactfourn_final."' WHERE rowid=".$id_poste;
			            dol_syslog(get_class($this)."::create", LOG_DEBUG);
			            $resql_up=$this->db->query($update_sql);
			            if (! $resql_up) $error++;
	            	}
	            	$tmpfactfourn_new = implode(",",$tmpfactfourn_scos); // return new fact_fourn
	            	//UPDATE fact_fourn table with new array
	            	$update_sql = 'UPDATE '.MAIN_DB_PREFIX."abcvc_projet_task SET fact_fourn='".$tmpfactfourn_new."' WHERE rowid=".$current_id_poste;
		            dol_syslog(get_class($this)."::create", LOG_DEBUG);
		            $resql=$this->db->query($update_sql);
		            if (! $resql) $error++;
	            }        	
            ...
            {
            	// var_dump("Nu avem valoare in current_id_poste");
	            // exit;
            	//in this case $current_id_poste don't exist and we need to upgrade DB with $id_poste
            	$fact_fourn_sql = 'SELECT fact_fourn from llx_abcvc_projet_task where rowid = '.$id_poste ;
            	// var_dump($fact_fourn_sql);
	            $resql_ff = $this->db->query($fact_fourn_sql);  
	            if ($resql_ff) {
	                $obj_result = $this->db->fetch_object($resql_ff);
            	...
            	$exp_factfourn = explode(",",$obj_result->fact_fourn);
            	$exp_factfourn[] = $this->id;
            	$imp_factfourn = implode(",",$exp_factfourn); // return new fact_fourn
            	//UPDATE fact_fourn table with new array
            	$update_sql = 'UPDATE '.MAIN_DB_PREFIX."abcvc_projet_task SET fact_fourn='".$imp_factfourn."' WHERE rowid=".$id_poste;
	            dol_syslog(get_class($this)."::create", LOG_DEBUG);
	            $resql=$this->db->query($update_sql);
	            if (! $resql) $error++;
            }
