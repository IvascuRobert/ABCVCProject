...
/**
 *	\file       htdocs/core/modules/propale/doc/pdf_abcvc.modules.php
 *	\ingroup    propale
 *	\brief      Fichier de la classe permettant de generer les propales au modele abcvc
 */
 ...
 */
class pdf_abcvc extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					// var_dump($object->lines[$i]->desc);
					// var_dump($object->lines[$i]->label);


					if($object->lines[$i]->label == ''  )
					{
						$task_ref = explode(' ', $object->lines[$i]->desc); // get all REF's from Sous poste & sous sous poste !
